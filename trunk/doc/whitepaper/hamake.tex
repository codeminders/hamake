...
vectors of all documents. For practical usage more advances clustering algorithms
...
First, we define a Map Reduce job that converts a book from
...
names, but within \textit{/plainText/} folder. With help of
...
similar books.
When new books are added, Hamake will not re-run DTRs: \textit{ExtractText}, 
\textit{Tokenize}, \textit{FilterStopWords} for all books. It will select
new books and add their terms to terms of other books. Next, it will 
re-calculate feature vector in DTR \textit{CalculateTF} and will
re-run DTRs: \textit{RunCanopyClusteringAlgorithm} and \textit{OutputResults}. 
In case new stop-words are added, Hamake will re-run DTRs: \textit{FilterStopWords}
\textit{CalculateTF}, \textit{RunCanopyClusteringAlgorithm} and \textit{OutputResults}
on full data set. 
