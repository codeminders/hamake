...
One way to measure similarity between two books is by measuring the cosine of the angle between their vectors\cite{wiki:cosinesimilarity}.
...
As a result, you will end up with 6 Map Reduce jobs:

Despite the fact that Mahout together with Hadoop simplifies a lot your job, you still should design and implement a code, that will
combine Map Reduce jobs in a single process, which will be triggered on incoming of new books. Also, you should implement a logic,
...
your Hadoop map-reduce jobs or Pig scripts in a workflow, that will be launched and controlled from a single machine. The order 
of execution of Hadoop map-reduce jobs or Pig scripts in hamake is based on dataflow programming model and is controlled by data, not by tasks themselfs.
