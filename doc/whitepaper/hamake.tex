...
could be referenced as DTR's input or output.
...
\textbf{hamake} attempts to ensure that all DTRs outputs are up to
date\footnote{\textbf{hamake} does not try to ensure that files which
  are not listed as one of DTR outputs are up to date, because it has
...
vertex is dependency graph. This allows to resolve cyclic dependencies
in a context of a single \textbf{hamake} execution.
...
are corresponding to two types of DTRs. Each of them have inputs,
outputs and processing instruction. Executing processing instruction
should bring DTR outputs up to date, compared to its
inputs. \emph{fold} implies many-to-one dependency between inputs and
the outputs. In other words, outputs depends on all inputs and if
either of inputs have been changed, outputs needs to be
updated. \emph{foreach} implies one-to-one dependency: where for each
file in input set there is a corresponding file in an output set, each
of them updated independently.
...
required computations in shortest possible time. To achieve this it
...
and \emph{reducer} tasks, each taking one respective slot. Number of
...
consider simple approach of using a \textit{vector space
...
depend on its inputs. For each book, they have to be invoked
...
as PDF to plain text. The input of the DTR is a reference to to
...
first search for DTRs that has no inputs or which inputs are not
outputs of other tasks. In our particular example, this is
\emph{ExtractPlainText}. \textbf{hamake} will launch corresponding
Hadoop jobs first, an as soon as they will be finished, it will
execute DTRs which depend on outputs of this DTR, and so on until all
...
into \textbf{hamake} dataflow requires use of \textit{generations} feature
