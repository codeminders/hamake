...
Hadoop is typically used to process large amounts of data through series of relatively simple operations. Usually Hadoop jobs are I/O-bound
...
We've striven to create a easy to use utility that developers can
...
\cite{hamakesyntax}. The two main elements, \emph{fold} and \emph{foreach}, correspond to two types of DTRs. Each element has input, output, and processing instructions. The execution of processing instructions brings DTR output up to date. 
...
\item[\emph{FindSimilar}] Run \textit{Canopy clustering algorithm}
  to group similar books into clusters using
...
dependencies could be represented by a directed acyclic graph, as shown in Figure~\ref{fig:SimilarityAlgDAG}, with vertices representing documents and jobs assigned to edges. The XML file describing this dataflow in \textbf{hamake} language is shown as
