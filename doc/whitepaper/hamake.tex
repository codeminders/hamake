...
of topological ordering in step 1. In step 6, when completed DTR is
...
files, not just DTRs. For example, if \emph{foreach} DTR \textit{B}
depends on \emph{foreach} DTR \textit{A}, it would allow to run some
of jobs from \textit{B} without waiting for all jobs in \textit{A} to
complete.
