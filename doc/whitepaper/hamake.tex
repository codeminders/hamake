...
\begin{abstract}
  ...
  datasets as vertices. Each edge indicates a dependency between two or
  more datasets and is associated with a processing instruction:
  a MapReduce job, PIG Latin script, or an external command,
...
\cite{hadoopattwitter},\cite{hs2010hadoopbench}. 
...
\textbf{hamake} was developed in late 2008. \textbf{hamake} is open
source, distributed under Apache License v2.0. The project is hosted
at google code at the following URL:
...
There are know approaches for estimating degree of similarity between
...
of machines. The problem could be split into series of steps, each of
them represented by a \textit{MapReduce job}:

\begin{enumerate}
\item Extract a plain text from native document format (e.g. PDF).
\item Split plain text into a keywords. Stemming and taking care of
  case differences, inflections, punctuation, accented characters,
  etc.
\item Filtering out \textit{stopwords} (e.g. words like \textit{a},
  \textit{the}, \textit{are}) from list of keywords.
\item For each text calculate a feature vector: a vector of
  frequencies of keywords.
\item Find books similar content by comparing their feature vectors
...
Extending notion of dependency checks beyond timestamp comparison,
with external dependency check scripts.
