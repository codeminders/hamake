...
Cyclic dependences has to be avoided, because such workflow is not
guaranteed to terminate. Implicit checks are perfomed during reading
hamakefile and building dependency graph. If cycle is detected, it is
reported as error. So the dependency graph used by hamake is
...
calculations: each run of hamake is a signle iteration. This mechanism
is implemented via hamake feature called \textit{generations}. Each
input or output file could be optionally marked with generation
...
Table~\ref{table:1} below attempts to compare Hamake and similar workflow
engines for Hadoop (\href{http://github.com/tucu00/oozie1}{Oozie},
...
Below are some notes, discussing some key differences between hamake and some other popular products:
