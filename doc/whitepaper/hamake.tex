...
consider simple approach of using a \textit{Canopy clustering
  algorithm}\cite{efficientClustering} on feature vectors of all
documents. The implementation could be split into series of steps,
each of them represented by a \textit{MapReduce job}:
...
\item[\emph{Tokenize}] Split plain text into a tokens, roughly
  ...
  accents and diacritics, case-folding, stemming or lemmatization of tokens,
  resulting in a list of \text{tokens}.
...
depends on input. For each book, they have to be invoked sequentially,
as outputs of one tasks are used as inputs of the next
...
file, where plain text version should be written.

In the same way, we add all five Map-Reduce jobs that are
left. \textbf{hamake} will determine jobs order by comparing their
input with output of other jobs.

\lstinputlisting[caption={\textit{hamakefile}, that describes process for
  detecting similar books}, label=hamakeFile]{sample.xml}

\textbf{hamake}, when launched with this XML descriptor it will first
search for tasks that has no inputs or which inputs are not outputs of
other tasks. In our particular example, this is job with id
...
list of similar books.  When new books are added, Hamake will not
re-run DTRs: \emph{ExtractText}, \emph{Tokenize},
\emph{FilterStopWords} for all books. It will select new books and add
their terms to terms of other books. Next, it will re-calculate
feature vector in DTR \emph{CalculateTF} and will re-run DTRs:
\emph{RunCanopyClusteringAlgorithm} and \emph{OutputResults}.  In case
new stop-words are added, Hamake will re-run DTRs:
\emph{FilterStopWords} \emph{CalculateTF},
\emph{RunCanopyClusteringAlgorithm} and \emph{OutputResults} on full
data set.
