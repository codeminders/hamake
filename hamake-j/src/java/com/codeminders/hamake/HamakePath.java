...
public class HamakePath {
	...
	public enum Variant {
		...
		public static Variant parseString(String variant) {
			if (StringUtils.isEmpty(variant))
				return null;
			if (variant.equalsIgnoreCase("list"))
				return LIST;
			else if (variant.equalsIgnoreCase("path"))
				return PATH;
			else if (variant.equalsIgnoreCase("mask"))
				return MASK;
			else
				return null;
		}
	...
	public HamakePath(String loc, String wdir, long validityPeriod, int gen)
			throws IOException {
	...
	public HamakePath(String ID, String wdir, String loc, String mask, int gen)
			throws IOException {
	...
	public HamakePath(String ID, String wdir, String loc, String filename,
			String mask, int gen, Variant variant, long validityPeriod)
			throws IOException {
		this.wdir = wdir;
		...
		if (filename != null && mask != null)
			throw new IllegalArgumentException(
					"Both filename and mask specified!");
	...
	public boolean intersects(HamakePath other) {
		return StringUtils.equals(this.getLoc(), other.getLoc())
				&& getGen() >= other.getGen()
				&& (getFilename() == null || other.getFilename() == null || StringUtils
						.equals(this.getFilename(), other.getFilename()));
	}
	...
	public FileStatus getFileStatus() throws IOException{
		return fs.getFileStatus(getPathName());
	}

	public List<HamakePath> getFileList(boolean create) throws IOException {
		if (Config.getInstance().test_mode)
			LOG.info("Scanning " + getPathName());
		
		if(filename != null) return Arrays.asList(this);

		boolean exists;

		synchronized (fs) {
			exists = fs.exists(getPathName());
		}
		if (!exists) {
			if (create) {
				LOG.info("Creating " + getPathName());
				synchronized (fs) {
					fs.mkdirs(getPathName());
				}
		...
		FileStatus stat;
		synchronized (fs) {
			stat = fs.getFileStatus(getPathName());
		}
		if (!stat.isDir()) {
			LOG.error("Path " + this + " must be dir!");
			return null;
		}

		FileStatus list[];
		synchronized (fs) {
			list = fs.listStatus(getPathName());
		}

		List<HamakePath> files = new ArrayList<HamakePath>();
		for (FileStatus s : list) {
			Path p = s.getPath();
			if (Utils.matches(p, mask))
				files.add(new HamakePath(ID, wdir, p.getParent().toString() , p.getName(), null, gen, null, validityPeriod));
		}
		return files;
	}
